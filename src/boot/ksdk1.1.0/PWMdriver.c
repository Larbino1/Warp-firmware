...
void enablePWMpins()
{
	...
	//PORT_HAL_SetMuxMode(PORTB_BASE, 5, kPortMuxAlt2);
	//PORT_HAL_SetMuxMode(PORTB_BASE, 6, kPortMuxAlt2);

	//CLOCK_SYS_TPM initialised in DRV_INIT
	TPM_DRV_Init(0U, (tpm_general_config_t *)&tpmConfig);
	TPM_DRV_SetClock(0U, kTpmClockSourceModuleMCGIRCLK, kTpmDividedBy4);
	TPM_DRV_Init(1U, (tpm_general_config_t *)&tpmConfig);
	TPM_DRV_SetClock(1U, kTpmClockSourceModuleMCGIRCLK, kTpmDividedBy4);
}
